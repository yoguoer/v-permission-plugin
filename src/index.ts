...
export function setupRouterGuard(options: PermissionGuardOptions) {
    ...
    const { router, whiteList, asyncRoutes, basicRoutes, getAuthList, domain } = options;
    // 使用参数
    createPermissionGuard(router, whiteList, asyncRoutes, basicRoutes, getAuthList, domain)
}
